 ...
 * [v3.2test1] RTSP Try Basic and if fail, try Digest.
 * [v3.2test2] attempt to fix something not working satisfactorily
 * [v3.2test3] Fix Auth Header parser overwriting correct auth params
 * [v3.2test4] Core dump (RTSP Try Basic and if fail, try Digest.)
 * [v3.2test5] Scheme for user to request to skip Basic Auth.
 */
#define VERSION "3.2.test5"
...
static int main_loop(struct ast_channel *chan,char *ip, int rtsp_port, char *url,char *username,char *password,int isIPv6,int sip_enable, char *cfg_realm_arg, int sip_port)
{
	...
	ast_log(LOG_NOTICE,"app_rtsp_sip version = %s\n", VERSION); /* added [3.1] */
        ...
        /* 
	 * v3.2 add hack to request Digest Only authentication
         */
	char cfg_realm[256];
	cfg_realm[0] = '\0';
	int  skip_basic=0;
	...
	strcpy(cfg_realm,"DS-022b77fb"); /* REMOVE THIS REMOVE THIS */
		...
		if(rchan !=NULL && outfd <0) /* Channel active */
		{
			...
			switch (player->state)
			{
				case RTSP_DESCRIBE:
					...
					if (responseCode==401)
					{
					    /* Test auth fail count, add line:  password="wrong"; */
 	                                    rtsp_auth_fail_count++; /* Added v3.2. Note: count=1 is normal */
					    ...
					    /* v3.2 provide ability to skip Basic Auth */
					        ...
					        ast_debug(3,"    - Checking for Auth Method of Basic\n");
 					        ...
 					        /*
						 * [v3.2] add auth fail counts.
						 * First try Basic Auth (if Basic Auth is available) 
						 *   and if fails a couple of times then try Digest.
						 *   and if Digest Auth is available and then fails a few times 
						 *   then bail out.
						 */ 
#ifdef TEST_BUFFER
						     ...
						     */
						   	...
						   	ast_debug(5,"  Challenge Response Data- rx_realm: %s nonce: %s\
						    	...
						    	break;
                                                    ...
                                                    {
					            	ast_debug(3,"    - RTSP: No Basic/Digest Method of Digest\n");
						      /*ast_log(LOG_ERROR,"No Basic/Digest Authentication data found for RTSP\n");*/
