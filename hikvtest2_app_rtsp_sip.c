 ...
 * [v3.2test2] fix bug where second auth meth overwrides params of first auth meth
 */
#define VERSION "3.2.test2"
        ...
        if (parse_headers(headers_start, &headers) == 0) //returns with 2xarray of parsed header key,value pairs
        {
            ...
            for (i = 0; i < headers.count; ++i) {
                ...
                if(strstr(headers.headers[i].field_name,"WWW-Authenticate")!=NULL)
                {
                    ...
                    while( (more_auths != NULL) && return_code !=0); /* v3.2 Stop if method found. */
                }
                if( return_code == 0 ) /* v3.2. If auth method found, done. Continuing find another method will overwrite params */
                    break;
            }
						...
						/* Check athentication method */
					        ...
					        ast_debug(3,"    - Checking for Auth Method of Basic\n");
 					        ...
 					        /*[v3.2] add auth fail counts */ 
                                                if ((GetAuthSchemeBasic(buffer,bufferLen,&basic_data) == 0) && (rtsp_auth_fail_count < 3) )
                                                {
						    ...
						    break;
                                                ...
                                                {
					            ast_debug(3,"    - No Auth Method of Basic (or failed too many times)\n");
                                                    /* 
						     ...
						     */
 						    ...
 						    /*[v3.2] add auth fail counts */ 
                                                    if ((GetAuthSchemeDigest(buffer,bufferLen,&digest_data) == 0) && (rtsp_auth_fail_count < 5) )
                                                    {
						...
						switch(responseCode){
							...
							case 401:
						                    ...
						                    ast_log(LOG_WARNING,"SIP Code does not support Basic Auth\n");
                                                              ...
                                                              /*else*//* v3.2 SIP always try Digest Method*/
								if(1) 
                                                                {
